...
\begin{itemize}
\item se $g$ \`e \textbf{crescente} su $S_X$: $g(X)\le y \iff X\le g^{-1}(y)$, quindi
\[ F_Y(y)=F_X\big(g^{-1}(y)\big) \]
\item se $g$ \`e \textbf{decrescente} su $S_X$: la disuguaglianza si ribalta, $g(X)\le y \iff X\ge g^{-1}(y)$, quindi
\[ F_Y(y)=1-F_X\big(g^{-1}(y)\big) \]
\item se $g$ \textbf{non \`e monotona} (es.\ $X^2$, $|X-a|$): la condizione si spezza in \textbf{due lati}, e la \textbf{cases} nasce da come questo intervallo interseca $S_X=[p,q]$: si individua la soglia $y^*$ in cui un bordo esce dal supporto (vedi i box specifici per $Y=X^2$ e $Y=|X-a|$).
\end{itemize}

\item Infine calcoliamo $f_Y(y)=F_Y'(y)$ derivando ogni pezzo separatamente (regola della catena: $\big(F_X(h(y))\big)'=h'(y)\,f_X(h(y))$). Controllo: $f_Y\ge0$, $\int f_Y=1$.
\end{enumerate}
\end{concetto}
...
Se la catena \`e \textbf{irriducibile}: $\vec v$ esiste ed \`e unica.
